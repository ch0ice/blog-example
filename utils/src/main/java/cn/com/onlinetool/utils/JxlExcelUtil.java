...
public class JxlExcelUtil {
     ...
     * @throws Exception
     */
    public static void exportExcelTemp(
            String fileName,
            String[] fields,
            HttpServletResponse response
    ) throws Exception {
        if(null == fields || 0 == fields.length){
            throw new Exception("导出excel模版，excel的列不能为空");
        }
        //设置默认文件名为当前时间：年月日时分秒
...
//        response.setContentType("multipart/form-data");
    ...
    /**
     * @param list       ：数据源
     * @param fieldMap   ：excel 列和对象属性的对应关系
     *                   例： fieldMap.put(orderId,"订单号") 或 fieldMap.put(order.orderId,"订单号")
...
//        response.setContentType("multipart/form-data");
    ...
    public static <T> void listToExcel(
        ...
        listToExcel(list, fieldMap, "sheet", 65535, response,fileName);
    }
     ...
     * @param in           ：Excel的输入流
     * @param sheetName    ：sheet页的名称
     * @param entityClass  ：对象属性和 excel列的对应关系 例： fieldMap.put("订单号","orderId")
     * @param fieldMap     ：Excel中的中文列头和类的英文属性的对应关系Map
     * @return ：List
     * @throws Exception
     * @Description : 将excel中指定的sheet页的数据转化为List
     */
     ...
     * @param in           ：excel的输入流
     * @param sheetName    ：sheet页的名称
     * @param entityClass  ：excel中每行数据对应的对象
     * @param fieldMap     ：对象属性和 excel列的对应关系 例： fieldMap.put("订单号","orderId")
     * @param uniqueFields ：指定业务主键组合（即复合主键），这些列的组合不能重复
     * @return ：List
     * @throws Exception
     * @Description : 将excel中指定的sheet页的数据转化为List,带去重
     */
    public static <T> List<T> excelToList(
            InputStream in,
            String sheetName,
     ...
     * @return ：List
     * @throws Exception
     * @Description : 将excel所有sheet页的数据转化为List
     */
    public static <T> List<T> excelToList(
            InputStream in,
            Class<T> entityClass,
        ...
        for(int l = 0; l < sheets.length; l++) {
            ...
            List<T> sheetResult = excelToList(wb,sheet.getName(),entityClass,fieldMap,null,false);
     ...
     * @return ：List
     * @throws Exception
     * @Description : 将excel所有sheet页的数据转化为List,带去重
     */
    public static <T> List<T> excelToList(
            InputStream in,
            Class<T> entityClass,
        ...
        for(int l = 0; l < sheets.length; l++) {
            Sheet sheet = sheets[l];
            List<T> sheetResult = excelToList(wb,sheet.getName(),entityClass,fieldMap,uniqueFields,true);
            resultList.addAll(sheetResult);
        }
        return resultList;
    }
    ...
    /**
     * @param wb : excel对象
     * @param sheetName    ：sheet页名称，只导入这一个sheet页
     * @param entityClass  ：excel中每行数据对应的对象
     * @param fieldMap     ：对象属性和 excel列的对应关系 例： fieldMap.put("订单号","orderId")
     * @param uniqueFields ：指定业务主键组合（即复合主键），这些列的组合不能重复
     * @param isDistinct   ：是否去除重复，不需要去重 uniqueFields 可以传 null
     * @return ：List
     * @throws Exception
     * @Description : 将excel中指定sheet页的数据转化为List
     */
    public static <T> List<T> excelToList(
            Workbook wb,
            String sheetName,
